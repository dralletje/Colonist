...
/*::
type Key = string
type Element = { key: Key, type: mixed };
type T_keyed_array = Array<Element>;
type T_old_map = Map<Key, Element>;
*/
// Gets the difference
...
export const render_parallel = (input$) => {
  /*:flow
  type Progress =
    | 'waiting'
    | 'working'
    ...
    start: listeners => {
      const continue_queue = () => {
        ...
        if (mounted.get(key).progress !== 'waiting') {
          console.log(chalk.blue(`NOT WAITING`), `instead: ${mounted.get(key).progress}`);
          console.log(chalk.blue(`NOT WAITING`), mounted.get(key));
        }

        // TODO Why is this mutating the item instead of the list?
        // mounted.set({ ...mounted.get(key), progress: 'working' }) ???
        mounted.get(key).progress = 'working';

        if (debug) console.log("START TASK")
        action(entry.props).addListener({
          ...
          complete: () => {
            ...
            if (debug) console.log('TASK DONE');
            setImmediate(() => {
              continue_queue();
            });
          }
